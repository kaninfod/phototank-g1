...
(function() {
  ...
  App.PhotoWidget = (function() {
    return {
      ...
      bindUIActions: function() {
        ...
        _this = this;
        this.modalInit();
        $(s.photoGrid).on('click.' + s.eventNamespace, '.lazy', function() {
          return _this.showInControlSidebar(this);
        });
        $(s.photoGrid).on('click.' + s.eventNamespace, '.overlay-button.overlay-select', function() {
          return _this.select(this);
        });
        $('.wrapper').on('click.' + s.eventNamespace, '.overlay-button.overlay-delete, #delete-photo', function() {
          return _this["delete"](this);
        });
        $(s.photoGrid).on('click.' + s.eventNamespace, '.overlay-button.overlay-zoom', function() {
          return _this.showModal(this);
        });
        $(s.photoGrid).on('click.' + s.eventNamespace, '.overlay-button.overlay-processing', function(ev) {
          return _this.reloadWidget(this);
        });
        $(s.photoGrid).on('mouseenter.' + s.eventNamespace, '.photo-widget', function() {
          return _this.showControls(this);
        });
        $(s.photoGrid).on('mouseleave.' + s.eventNamespace, '.photo-widget', function() {
          return _this.hideControls(this);
        });
        $('body').on('click.' + s.eventNamespace, '#add-to-album-photo', function() {
          return _this.showAddToAlbum();
        });
        $('#album-list-photo .btn-primary').on('click', function(event) {
          return _this.addToAlbum(event);
        });
        return $('body').on('click.' + s.eventNamespace, '#rotate-photo', function() {
          return _this.rotatePhotos(this);
        });
      },
      reloadWidget: function(element) {
        var photoId, photoWidget, url;
      ...
      rotatePhotos: function(element) {
        var degrees, photoId, processingButton;
        ...
        $.get('/photos/' + photoId + '/rotate/' + degrees, function(data) {
          processingButton.addClass('overlay-show');
          $('.dropdown.open .dropdown-toggle').dropdown('toggle');
          return alertify.log("Photo is queued for a rotation of " + degrees + " degrees");
        });
        return false;
      },
      showInControlSidebar: function(element) {
        var photoId;
        ...
        return $(element).parents('.photo-widget').addClass('highlight');
      },
      select: function(element) {
        var photoId;
        photoId = $(element).parents('.photo-widget').data("photoid");
        return App.Bucket.addPhotoToBucket(photoId);
      },
      "delete": function(element) {
        var photoId, photoWidget;
        console.log('kaj');
      ...
      deletePhoto: function(photoId) {
        ...
        alertify.confirm('Really! Delete this Photo?', (function() {
          var url;
          ...
          return $.ajax({
            ...
            success: function(data) {
              ...
              return alertify.log("Photo is queued for deletion!");
            }
          });
        }));
        return false;
      ...
      showAddToAlbum: function() {
        ...
        return $('#album-list-photo').removeClass('hidden');
      },
      addToAlbum: function(event) {
        var album_id, photoId;
        album_id = $('#album-list-photo * #albums input:radio:checked').val();
        photoId = $('#photo_id').data("photo_id");
        return $.get('/albums/add_photo', {
          album_id: album_id,
          photo_id: photoId
        }, function() {
          return alertify.log("Photo was added to album");
        });
      },
      showControls: function(element) {
        var overlayButton;
        overlayButton = $('.overlay-button:not(.overlay-processing)', element);
        return overlayButton.addClass('overlay-show');
      },
      hideControls: function(element) {
        var overlayButton;
        overlayButton = $('.overlay-button:not(.selected, .overlay-processing)', element);
        return overlayButton.removeClass('overlay-show');
      },
      modalInit: function() {
        return this.modal = new AnimatedModal({
          animatedIn: 'zoomIn',
          animatedOut: 'zoomOut',
      ...
      showModal: function(element) {
        var _this, photoId, url;
        _this = this;
        photoId = $(element).parents('.photo-widget').data("photoid");
        url = '/photos/' + photoId + '?view=modal';
        return $('#photoDetails > .modal-content').load(url, function(result) {
          return _this.modal.open();
        });
      },
      getWidget: function(photoId) {
  ...
  $(document).on("page:change", function() {
    if (!($(".photos.index, .catalogs.show, .albums.show, .locations.show").length > 0)) {
      return;
    }
    return App.PhotoWidget.init();
  });
  ...
  App.PhotoLike = (function() {
    return {
      ...
      bindUIActions: function() {
        ...
        return $('body').on('click.' + s.eventNamespace, s.likeButtonId, function() {
          return _this.likePhoto($('#photo_id').data("photo_id"));
        });
      },
      likePhoto: function(photoId) {
        ...
        return $.get(url, function(data) {
          ...
          return $(s.likeButtonId).toggleClass('btn-success');
        });
      }
    };
  })();
  ...
  App.PhotoComment = (function() {
    return {
      ...
      bindUIActions: function() {
        var _this;
        _this = this;
        return $('body').on('keypress', s.commentInput, function(event) {
          return _this.addComment(event, this);
        });
      },
      addComment: function(event, input) {
        var url;
        if (event.which === 13) {
          url = this.getUrl();
          $.get(url, {
            ...
            return $(msg).hide().prependTo(s.commentsBox).fadeIn(1000);
          });
          $(input).val('');
          return false;
        }
      ...
      getUrl: function() {
        ...
        return url = '/photos/' + photo_id + '/add_comment';
      }
    };
  })();
...
(function() {
  ...
  App.PhotoTaginput = (function() {
    return {
      ...
      refresh: function() {
        return this.initTaginput();
      },
      bindUIActions: function() {
        var _this;
        _this = this;
        $('body').on('itemAdded.' + s.eventNamespace, '.tags', function(event) {
          return _this.addTag(event);
        });
        $('body').on('beforeItemRemove.' + s.eventNamespace, '.tags', function(event) {
          return _this.removeTag(event);
        });
        return $('.bootstrap-tagsinput > input').autocomplete({
          source: '/photos/get_tag_list',
          minLength: 1
        });
      },
      addTag: function(event) {
        var photo_id, tag, url;
        photo_id = $('#photo_id').data("photo_id");
        url = '/photos/' + photo_id + '/addtag';
        tag = {
          tag: event.item
        };
        return $.get(url, tag, function(data) {
          return $('.bootstrap-tagsinput input').val('');
        });
      },
      ...
      initTaginput: function() {
        $(s.taginput).tagsinput({
          tagClass: function(item) {
            switch (item.charAt(0)) {
              case '@':
                return s.genericClass;
              default:
                return s.mentionClass;
            }
          },
          trimValue: true
        });
        return $('.bootstrap-tagsinput > input').autocomplete({
          ...
          select: function(event, ui) {
            return $(s.tagInput).tagsinput('add', ui.item.value);
          }
        });
      }
    };
